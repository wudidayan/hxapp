...
- (TDView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image;
...
- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position {
    TDView *toast = [self viewForMessage:message title:nil image:nil];
    
    [self showToast:toast duration:interval position:position];
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position title:(NSString *)title {
    TDView *toast = [self viewForMessage:message title:title image:nil];
    [self showToast:toast duration:interval position:position];
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position image:(UIImage *)image {
    TDView *toast = [self viewForMessage:message title:nil image:image];
    [self showToast:toast duration:interval position:position];
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval  position:(id)position title:(NSString *)title image:(UIImage *)image {
    TDView *toast = [self viewForMessage:message title:title image:image];
    [self showToast:toast duration:interval position:position];
}
...
- (void)showToast:(TDView *)toast duration:(CGFloat)interval position:(id)point {
    toast.center = [self centerPointForPosition:point withToast:toast];
    toast.alpha = 0.0;
    NSMutableArray *array=[[NSMutableArray alloc]initWithCapacity:10];
    for(UIView *view in self.subviews){
        if ([view isKindOfClass:[TDView class]]) {
...
- (void)makeToastActivity:(id)position {
    ...
    //L
    activityView.backgroundColor = [[UIColor colorWithRed:130/255.0 green:170/255.0 blue:170/255.0 alpha:1.0] colorWithAlphaComponent:CSToastOpacity];
...
- (TDView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    TDView *wrapperView = [[[TDView alloc] init] autorelease];
    ...
    wrapperView.backgroundColor = [[UIColor colorWithRed:70/255.0 green:70/255.0 blue:72/255.0 alpha:1.0] colorWithAlphaComponent:1.0];
