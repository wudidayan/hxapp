...
@interface TDTYPayViewController ()
{
    NSString *_termNo;
    NSString *_track;
    NSString *_pinblk;
...
@implementation TDTYPayViewController
#pragma mark - 属性们懒加载
...
- (void)viewDidLoad {
    ...
    self.amtLabel.text = self.payInfo.payAmt;
...
-(void)payBtnClick:(UIButton *)sender
{
    ...
    {
        TDSignViewController *signVC = [[TDSignViewController alloc]init];
        signVC.payInfo = self.payInfo;
...
//        
//        NSLog(@"TDK写入成功");
//        NSString *amt = [NSString stringWithFormat:@"%ld",(long)(self.amtLabel.text.doubleValue * 100)];
//        NSLog(@"%@",amt);
//        [self.tyMos readCard:amt TerminalTime:dateStr TradeType:0x00 timeout:60 Demote:nil];
