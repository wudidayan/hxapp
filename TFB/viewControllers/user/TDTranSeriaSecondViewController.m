//
//  TDTranSeriaSecondViewController.m
//  TFB
//
...
@interface TDTranSeriaSecondViewController ()
{
    int page;
...
@implementation TDTranSeriaSecondViewController
...
//    [self.successArray removeAllObjects];
     ...
     */
    NSString *pageStr = [NSString stringWithFormat:@"%d", page];
    [TDHttpEngine requestForGetTranSerialListWithCustId:[TDUser defaultUser].custId custMobile:[TDUser defaultUser].custLogin busType:_tranMeString payWay:@"" tranState:@"" startTime:@"" endTime:@"" startLineNum:pageStr pageSize:@"15" recordType:@"1" complete:^(BOOL succeed, NSString *msg, NSString *cod, NSArray *temArray) {
        [self.tableView.header endRefreshing];
        if (succeed) {
            NSLog(@"shuju: %@", temArray);
            if (temArray.count == 0) {
            ...
            [self.dataMuArray addObjectsFromArray:temArray];
            NSLog(@"%@",self.dataMuArray);
            for (TDTranSerial *tranSerial in self.dataMuArray) {
                NSLog(@"%@",tranSerial.ordMessage);
            }
...
//    NSLog(@"%d",self.successArray.count);
    if(indexPath.row <= self.dataMuArray.count) {
        ...
        NSLog(@"%@", [NSString stringWithFormat:@"indexPath.row %ld > dataMuArray.count %lu", (long)indexPath.row, (unsigned long)self.dataMuArray.count]);
    }
